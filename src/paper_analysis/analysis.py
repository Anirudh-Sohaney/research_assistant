...
def _judge_prompt(name: str, rubric: str, selected_text: str, retry: bool = False) -> List[Dict[str, str]]:
    system = (
...
async def _run_judge(name: str, rubric: str, selected_text: str) -> JudgeResult:
    ...
    for attempt, max_tokens in enumerate((4000, 5200, 6200, 7000)):
        payload = RequestPayload(
            ...
            json_body={
                "model": DEFAULT_MODEL,
                "messages": _judge_prompt(name, rubric, selected_text, retry=attempt > 0),
                "temperature": 0.2,
